...
async function testPdf(html, name, mode = 'print') {
  ...
  try {
    ...
    writeFileSync(`/data/gloria-cloud/html-editor/test_output/v3_${name}_${mode}.pdf`, Buffer.from(buf));
...
<div class="footer">本报告仅供参考，不构成投资建议</div>
  ...
  {
    name: 'overflow_scroll容器',
    html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
body{margin:0;font-family:sans-serif}
.container{width:1000px;margin:0 auto;padding:40px}
.scroll-box{max-height:300px;overflow:auto;border:1px solid #ccc;padding:20px;background:#f9f9f9}
</style></head><body>
<div class="container">
<h1>overflow:auto测试</h1>
<div class="scroll-box">
${Array(20).fill(0).map((_,i) => `<p>这是第${i+1}行内容，用于测试overflow:auto容器在PDF中是否正确显示全部内容。</p>`).join('')}
</div>
<p>下面还有更多内容...</p>
${Array(5).fill(0).map((_,i) => `<p>额外段落 ${i+1}。</p>`).join('')}
</div>
</body></html>`
  },
  {
    name: '连续长文不分页',
    html: `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
body{margin:0;font-family:sans-serif;max-width:900px;margin:0 auto;padding:40px;line-height:1.8;color:#333}
h1{color:#1a1a2e;border-bottom:3px solid #4f46e5;padding-bottom:14px}
p{margin-bottom:16px}
</style></head><body>
<h1>长文档测试</h1>
${Array(30).fill(0).map((_,i) => `<p>这是第${i+1}段。这是一段测试文字，用于验证文档型PDF生成效果。页面长度适配应该能够正确处理多页内容，确保文字不被切断，分页位置合理。内容应该连续流动，不应该出现大片空白。</p>`).join('')}
</body></html>`
  }
];
